		...
		if((*fmt) == '%')
		{
			...
			else if((*fmt) == 'l')
			{
				...
				if((*fmt) == 'l')
				{
					...
					{
						unsigned long long u = __builtin_va_arg(args, unsigned long long);
						char temp[32] ={'0'};
						llu_to_hex_str(u,temp,false,true);
						uart_puts(temp);
						fmt++;
					}
					else if((*fmt) == 'X')
					{
						unsigned long long u = __builtin_va_arg(args, unsigned long long);
						char temp[32] ={'0'};
						llu_to_hex_str(u,temp,true,true);
						uart_puts(temp);
						fmt++;
					}	
				}
			...
			else if((*fmt) == 'x')
			{
				unsigned long long u = __builtin_va_arg(args, unsigned long long);
				char temp[32] ={0};
				llu_to_hex_str(u,temp,false,false);
				uart_puts(temp);
				fmt++;
			}
			else if((*fmt) == 'X')
			{
				unsigned long long u = __builtin_va_arg(args, unsigned long long);
				char temp[32] ={0};
				llu_to_hex_str(u,temp,true,false);
				uart_puts(temp);
				fmt++;
			}
		}
...
static void _memset_1Byte(char* dst, int c, size_t count)
{
	while(count --){
		*dst++ = c; 
	}
...
static void _memset_16Byte(char* dst, unsigned long long data, size_t count)
{
	unsigned long long end = (unsigned long long) dst + count;
...
void memset(void* dst, int c, size_t count)
{
	...
	unsigned long long addr = (unsigned long long) dst;

	c &= 0xff;
	for(int i = 0; i < 8; i++)
		data |= ((unsigned long long)c) << (i * 8);

	//The dst is not align with 16 byte
	if(addr & (align - 1)){
		unsigned long long next_addr = (addr + (align - 1)) & ~(align - 1);
		size_t s = (next_addr - addr) >= count ? count : (next_addr - addr);

		_memset_1Byte(dst_char,c,s);
	...
	if(count > 0)
	{
		...
		if(n > 0){
			_memset_16Byte(dst_char,data,align*n);
			dst_char += align*n;
		}
		
		if(count > 0)
			_memset_1Byte(dst_char,c,count);
	}	
}
