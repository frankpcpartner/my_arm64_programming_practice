...
{
    int pid = do_fork(PF_KTHREAD,(unsigned long long)&kernel_thread1,0);

	if(pid < 0)
	{
		printf("\rcreate kernel_thread1 failed.\n");
		return;
	}

	pid = do_fork(PF_KTHREAD,(unsigned long long)&kernel_thread2,0);

	if(pid < 0)
	{
		printf("\rcreate kernel_thread2 failed.\n");
		return;
	}
}
