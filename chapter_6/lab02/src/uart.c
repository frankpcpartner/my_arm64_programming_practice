...
static enum UART_TYPE g_cur_uart = type_mini_uart;
...
{
	g_uart[g_cur_uart].ops.send(c);	
}

char uart_getc()
{
	return g_uart[g_cur_uart].ops.getc();
}
